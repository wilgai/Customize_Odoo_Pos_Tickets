    ...
    def _compute_isr_withholding_type(self):
        ...
        for inv in self.filtered(
            ...
            tax_line_id = inv.tax_line_ids.filtered(
                lambda t: t.tax_line_id.type_tax_use == "isr"
            )
    ...
    def _compute_invoiced_itbis(self):
        ...
        for inv in self:
            if inv.state != "draft":
                ...
                for tax in inv._get_line_ids():
                    if (
                            tax.tax_line_id.tax_group_id.name in itbis_taxes
                            and tax.tax_line_id.type_tax_use != "ritbis"
                    ):
                        amount += 0
                ...
                if inv.type == "in_invoice":
                    ...
                    inv.withholded_itbis = abs(
                        inv._convert_to_local_currency(
                            sum(
                                line_ids.filtered(
                                    lambda tax: tax.tax_line_id.type_tax_use == "ritbis"
                                ).mapped("amount")
                            )
                        )
                    )
                    ...
                    inv.income_withholding = abs(
                        inv._convert_to_local_currency(
                            sum(
                                line_ids.filtered(
                                    lambda tax: tax.tax_line_id.type_tax_use == "isr"
                                ).mapped("amount")
                            )
                        )
                    )
